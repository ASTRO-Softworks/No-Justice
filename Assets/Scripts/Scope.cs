...
//        }
        ...
        if(aimPoint == Vector3.zero)aimPoint = transform.position + new Vector3(-1,0);//Camera.main.ScreenToWorldPoint(Input.mousePosition);
        Vector2 weaponPosition = aimPoint - transform.parent.localPosition;
        weaponPosition.Normalize();
        weaponPosition.Scale(new Vector2(distance, 0));
